...
import EmonaIcon from "./components/SvgIcons/EmonaIcon";
...
export default function Home() {
  ...
  const handleGameJoin = (name?: string) => {
    setUserName(name);
    ...
    setShowSuccessScreen(false);
      ...
      return (
        <>
          ...
          {showSuccessScreen && <SuccessScreen onComplete={handleSuccessComplete} userName={userName} />}
          
          {/* Gifts Button */}
          {!isLoading && !showSuccessScreen && <Gifts />}

          {/* Spinning SVG Background - Always visible */}
      {isMounted && (
      ...
      <div 
        className={`min-h-screen max-w-5xl mx-auto flex flex-col items-center justify-center relative overflow-hidden transition-all duration-1000 ease-out z-10 ${
      ...
      >
        ...
        <div className="w-full max-w-lg lg:max-w-xl xl:max-w-2xl mx-auto px-5 py-6 relative z-20">
          <Navbar />
          <Main onGameJoin={handleGameJoin} />
        </div>
      </div>
